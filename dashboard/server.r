...
function(input, output, session) {
  ...
  observeEvent(c(input$countrySelect), {
    ...
    if(country_name != ""){
      ...
      output$ROWclass <- renderText({paste("Current Extended RoW Classification: ", tail(extended_row_dat[extended_row_dat$country_name == country_name, c("Extended RoW Classification")], 1), sep ="")})
